...
func (s *Store) SaveObservation(ctx context.Context, v model.Observation, fingerprint string) error {
	_, e := s.db.ExecContext(ctx, `INSERT INTO observations(id,period_id,fingerprint,body) VALUES(?,?,?,?) ON CONFLICT(period_id,fingerprint) DO UPDATE SET id=excluded.id,body=excluded.body`, v.ID, v.PeriodID, fingerprint, encode(v))
	return e
}

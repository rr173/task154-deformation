...
func (v *Service) Compute(ctx context.Context, id string) (model.Result, error) {
	...
	r := model.Result{ID: uuid.NewString(), PeriodID: id, Version: len(prior) + 1, Points: out, CreatedAt: v.now().UTC(), ObservationCount: adjustment.ObservationCount(obs), MaxResidual: maxResidual}
